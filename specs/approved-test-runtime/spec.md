...
declaration at `DevHarness/local-projects/example-consumer/devharness.yaml`, reported at 100/100
structural coverage with proposal SHA-256 bound by the developer review service. It is deliberately
outside the example-consumer repository and ignored by DevHarness Git.

## Decision

The developer approved Docker as example-consumer's standard autonomous test runtime: complete local
...
Given the approved strategy, when the external declaration is compiled against a clean example-consumer
...
Given this implementation, when it is validated, then example-consumer containers, services and tests are not
...
- Running example-consumer before the required local capabilities are approved.

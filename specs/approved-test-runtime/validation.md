...
- AC-3 PASS (structural): the external example-consumer declaration validates against the public
...
- AC-4 PASS: validation did not invoke Docker Compose, example-consumer services, Playwright or consumer test
  commands. example-consumer has zero tracked or untracked changes and contains no `devharness.yaml`.
...
- example-consumer declaration structural coverage: 100/100.
- example-consumer declaration blockers: 0.
- example-consumer configured launch surfaces: 1/1 lifecycle-bound.
- example-consumer configured behavior jobs: 1/1 service-bound.
...
testing with no example-consumer file changes. DevHarness therefore loads the exact declaration from its
...
performed without modifying example-consumer. The runtime result is intentionally not a passing claim:
...
`docs/dogfood/discovery-doctor-dogfood.md`.
